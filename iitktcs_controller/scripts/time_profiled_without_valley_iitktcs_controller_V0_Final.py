...
class Success(smach.State):
    ...
    def execute(self, userdata):
        if(userdata.non_target_object==False):
            ...
            userdata.json_write_available_objects[userdata.bin].remove(userdata.target_object)
            ...
            userdata.json_write_available_objects[userdata.bin].remove(userdata.target_object)
            userdata.json_write_available_objects[userdata.temp_object_placement].append(userdata.target_object)
...
def main():
    ...
    temp_object_placement_joints=[[[-0.011978, -1.09605, 1.85554, -2.30167, -1.5791, -0.0115073],[0.0942938, -1.21103, 2.04125, -2.37304, -1.57442, 0.0942627]],
                                  [[-0.500536, -1.08686, 1.82977, -2.29352, -1.58087, -0.467904],[-0.508391, -1.24739, 2.08547, -2.38858, -1.57872, -0.476082]]]
    ...
    sm.userdata.json_write_available_objects= []
    ...
    with sm:
        ...
        def read_object_list_from_json_response_cb(userdata, response):
            ...
            userdata.json_write_available_objects = userdata.bin_available_objects
            temp_bin_available = [[],[]]
            for i in range(0,len(userdata.bin_available_objects)):
        ...
        def robot_retrieval_request_cb(userdata, request):
            ...
            if (userdata.non_target_object == False):
                retrieval_request.change_bin.data = False
            else:
                retrieval_request.change_bin.data = True
            userdata.state_start_time = present_time_in_millisec().getTime()
            print "Retrieval request is {}.".format(retrieval_request)
        ...
        def json_writter_request_cb(userdata,request):
            ...
            for i in range(0,len(userdata.json_write_available_objects)):
                bin_available_ids = custom_ids()
                bin_available_ids.ids.data = userdata.json_write_available_objects[i]
