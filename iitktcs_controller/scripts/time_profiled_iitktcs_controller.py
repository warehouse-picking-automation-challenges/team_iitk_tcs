...
def main():
    ...
    with sm:
        ...
        def single_goal_response_cb(userdata, response):
            userdata.state_stop_time = present_time_in_millisec().getTime()
            print "Time taken for Drop Position One state is {} microsec.".format(userdata.state_stop_time-userdata.state_start_time)
            print " "
            time.sleep(3)
            if (userdata.debug == True):
        ...
        def valley_detection_response_cb(userdata, response):
            userdata.state_stop_time = present_time_in_millisec().getTime()
            print "Time taken for Valley Detection State is {} microsec.".format(userdata.state_stop_time-userdata.state_start_time)
            print " "
            if (userdata.debug == True):
                print response
        ...
        def drop_to_valley_response_cb(userdata, response):
            userdata.state_stop_time = present_time_in_millisec().getTime()
            print "Time taken for Drop To Valley state is {} microsec.".format(userdata.state_stop_time-userdata.state_start_time)
            print " "
            if (userdata.debug == True):
                print response
            if(response.success.data == True):
        ...
        def single_goal_response_cb(userdata, response):
            userdata.state_stop_time = present_time_in_millisec().getTime()
            print "Time taken for Valley Detection Failure State is {} microsec.".format(userdata.state_stop_time-userdata.state_start_time)
            print " "
            if (userdata.debug == True):
