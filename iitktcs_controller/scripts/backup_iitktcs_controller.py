...
class target_object_check(smach.State):
    ...
    def execute(selfself, userdata):
        print "Target object is : {}".format(userdata.bin_target_objects)
...
class reattempt(smach.State):
    ...
    def execute(self, userdata):
        ...
        if(size == 0):
            return 'rest'
        elif(userdata.remove_non_target_object == 1):
...
def main():
    ...
    # Drop joint angles
    drop_joints = [[-1.30076, -1.15654, 1.65536, -1.87283, -1.71668, 1.01511],
                   [1.03473, -1.30957, 1.99241, -2.06399, -1.56012, 0.489856],
                   [-1.96439, -1.30955, 1.99248, -2.06398, -1.5601, 0.489856],
                   [-1.96439, -1.30955, 1.99248, -2.06398, -1.5601, 0.489856],
                   [-1.96439, -1.30955, 1.99248, -2.06398, -1.5601, 0.489856]]
    ...
    with sm:
            ...
            # For non-target objects.
                ...
                for i in range(0,len(userdata.bin_available_objects[userdata.bin])):
                    if not userdata.bin_available_objects[userdata.bin][i] in userdata.bin_target_objects[userdata.bin]:
                        object_det.ids_target.data.append(userdata.bin_available_objects[userdata.bin][i])
                # object_det.ids_target.data          =list(set(userdata.bin_available_objects[userdata.bin])-set(userdata.bin_target_objects[userdata.bin]))
        ...
        def object_detect_service_response_cb(userdata, response):
            ...
            if(userdata.non_target_object==False):
                ...
                #     temp.append([userdata.object_info[i],len(userdata.object_info[i].obj_info_vect)])
                # temp = sorted(temp, key=itemgetter(1))
                # userdata.object_info = []
                # for i in range(0,len(temp)):
                #     userdata.object_info.append(temp[i][0])

                # Sorting done
                # print userdata.object_info
                temp_available_object_ids   = []
                temp_unavailable_object_ids = []
                print "Length is {}".format(len(userdata.object_info))
                for i in range(0,len(userdata.object_info)):
                ...
                # temp_unavailable_object_ids = list(set(userdata.bin_target_objects[userdata.bin])-set(temp_available_object_ids))
                ...
                if len(userdata.object_info) != 0:
                    for i in range (0,len(userdata.object_info)):
                        ...
                        for j in range (0,len(userdata.object_info[i].obj_info_vect)):
                            print userdata.object_info[i].obj_info_vect[j].id.data
                        print "@@@@@@@@@@@@"
